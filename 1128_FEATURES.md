...
|---|---|---:|---|
| F1 | **Multi-language Unit Test Generator** — extend `c_test_generator` to Java / Python / JS / TS / .NET / Kotlin / Swift / ABAP; mocks, parameterized, edge cases | 17 | Reuses `chunk_file` and AST tools; per-language framework adapters (pytest, JUnit, xUnit, Jest, …) — ✅ **SHIPPED (Phase 5)** as `agents/unit_test_generator.py` |
| F2 | **Test Case Generator from User Stories** — intake from Jira / Azure DevOps / Qase; outputs positive, negative, functional, NFR cases | 77 | Needs Jira/ADO fetcher alongside existing GitHub/Gitea — ✅ **SHIPPED (Phase 5)** as `agents/story_test_generator.py` |
| F3 | **BDD / Gherkin Feature File Generator** — Given-When-Then; Cucumber / Robot Framework output | 4 | Shares spec-ingestion with F2 — ✅ **SHIPPED (Phase 5)** as `agents/gherkin_generator.py` |
...
| F5 | **API Test Generator from OpenAPI/Swagger** — REST Assured, xUnit, Postman collections | 16 | Fits neatly beside existing API Doc Generator — ✅ **SHIPPED (Phase 6 Wave 6A)** as `agents/api_test_generator.py` + `tools/postman_emitter.py` |
...
| ID | Feature | UCs | Notes |
|---|---|---:|---|
| F20 | **CI/CD-Triggered Review Webhook** — Jenkins, GitHub Actions, Bitbucket, GitLab, Azure DevOps | 31 | Today reviews are user-invoked in Streamlit — this adds server mode — ✅ **SHIPPED (Phase 5, tool)** as `tools/webhook_server.py` |
| F21 | **Pre-Commit / Staged-Change Reviewer** — git hook; reviews only the diff | 2 | ✅ **SHIPPED (Phase 5, tool)** as `tools/precommit_reviewer.py` |
...
| F24 | **API-Contract Conformance Checker** — code ↔ declared Swagger/OpenAPI | 2 | ✅ **SHIPPED (Phase 5)** as `agents/api_contract_checker.py` |
...
| F37 | **Swagger/OpenAPI Generator from Code OR Requirements** — bidirectional | 2 | Extends existing API Doc Generator — ✅ **SHIPPED (Phase 5)** as `agents/openapi_generator.py` |
